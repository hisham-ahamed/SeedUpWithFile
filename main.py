...
from torrent_downloader import (
    download_torrent, get_download_status, clear_session, print_torrent_files
)
...
Examples:
  ...
  python main.py list-files -t movie.torrent
  python main.py list-files -t "magnet:?xt=urn:btih:..."

  # Download selected files 1, 3, and 7 through 10
  python main.py download -t movie.torrent --files "1,3,7-10"
...
Just run your commands directly - no manual setup required!
    ...
    download_parser.add_argument(
        ...
        type=str,
        default='',
        help="1-based file selection, e.g. '1,3,7-10' (blank downloads all)"
    )
    
    # List torrent files command
    list_parser = subparsers.add_parser(
        'list-files',
        help='Show numbered files inside a torrent or magnet link'
    )
    ...
    list_parser.add_argument(
        ...
        type=int,
        default=180,
        help='Seconds to wait for magnet metadata (default: 180)'
    )
...
def handle_list_files(args):
    """List numbered files contained in a torrent."""
    try:
        print_torrent_files(
            args.torrent,
        ...
        logger.error(f"Could not list torrent files: {e}")
        return 1
